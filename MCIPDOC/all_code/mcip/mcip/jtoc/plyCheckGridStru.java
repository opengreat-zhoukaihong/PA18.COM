package jtoc;

public class plyCheckGridStru{

    public java.lang.String c_crt_cde;     /*创建人员*/
    public java.lang.String c_crt_tm;      /*创建时间*/
    public java.lang.String c_upd_cde;     /*修改人员*/
    public java.lang.String c_upd_tm;      /*修改时间*/
    public java.lang.String c_ply_app_no;  /*单据号*/
    public java.lang.String c_send_tm;     /*发送时间*/
    public java.lang.String c_dpt_cde;     /*所属机构*/
    public java.lang.String c_source;      /*产生源[0]='0'*/
    public java.lang.String c_send_dpt_cde;/*网站编码*/
    public java.lang.String c_send_cde;    /*发送人*/
    public java.lang.String c_des_dpt_cde; /*目标机构*/
    public java.lang.String c_emergency;   /*紧急程度*/
    public java.lang.String c_authprn_cde; /*授权人*/
    public java.lang.String c_udr_mrk;     /*核保标志[0]='1'*/
    public java.lang.String c_send_mrk;    /*发送标志[0]='0'*/
    public java.lang.String c_udr_opn_mrk; /*核保意见标志*/
    public java.lang.String c_vrfy_mrk;    /*审核标志*/
    public java.lang.String c_mdfy_tm;     /*修改时间*/
    public java.lang.String c_insrnc_cde;  /*险种*/
    public java.lang.String c_undr_prsn;   /*核保人*/
    public java.lang.String c_undr_dpt;    /*核保机构*/
    public java.lang.String c_prn_typ;     /*单证类型[0]='5'*/
    public java.lang.String c_rinsrnc_mrk; /*分保类型*/
    public java.lang.String c_coinsrnc_mrk;/*共保标志*/
    public java.lang.String c_ply_no;      /*保单号*/
    public java.lang.String c_edr_no;      /*批单号*/
    public java.lang.String c_read;        /*读取标志  0-投保单未读入 1-投保单已读入 */
    public java.lang.String c_flag;        /*提交处理标志=7*/
    public java.lang.String c_insrnt_nme;  /*被保险人名称*/
    public java.lang.String c_insrnc_cnm;  /*险种名称*/
    public java.lang.String c_dpt_cnm;     /*所属机构名称*/
    public java.lang.String c_send_dpt_cnm;/*发送机构名称*/
    public java.lang.String c_des_dpt_cnm; /*目标机构名称*/
    public java.lang.String c_undr_dpt_cnm;/*核保机构名称*/
    public java.lang.String t_sign_tm;     /*签单日期*/
    public java.lang.String c_rpt_desc;    /*上报说明*/
    public java.lang.String c_edr_typ;     /*批改类型*/
    public java.lang.String c_risk_lvl_cde;/*风险等级代码*/
    public java.lang.String c_lcn_no;      /*牌照号码*/
    public java.lang.String c_eng_no;      /*发动机号*/
    public java.lang.String c_insrnc_cls;  /*险种大类*/
    public java.lang.String c_use_atr;     /*使用性质*/
    public java.lang.String c_app_typ;     /*投保分类*/
    public java.lang.String t_app_tm;      /*投保日期*/
    public java.lang.String t_input_tm;    /*录入日期*/
    public java.lang.String c_app_nme;     /*投保人名称*/
    public java.lang.String c_undr_opn;    /*核保意见*/
    public java.lang.String t_edr_app_tm;  /*申请日期*/
    public java.lang.String c_internet_typ;/*网上业务标志[0]='2'*/

    public plyCheckGridStru(){
          c_crt_cde="";     /*创建人员*/
          c_crt_tm="";      /*创建时间*/
          c_upd_cde="";     /*修改人员*/
          c_upd_tm="";      /*修改时间*/
          c_ply_app_no="";  /*单据号*/
          c_send_tm="";     /*发送时间*/
          c_dpt_cde="";     /*所属机构*/
          c_source="";      /*产生源[0]='0'*/
          c_send_dpt_cde="";/*网站编码*/
          c_send_cde="";    /*发送人*/
          c_des_dpt_cde=""; /*目标机构*/
          c_emergency="";   /*紧急程度*/
          c_authprn_cde=""; /*授权人*/
          c_udr_mrk="";     /*核保标志[0]='1'*/
          c_send_mrk="";    /*发送标志[0]='0'*/
          c_udr_opn_mrk=""; /*核保意见标志*/
          c_vrfy_mrk="";    /*审核标志*/
          c_mdfy_tm="";     /*修改时间*/
          c_insrnc_cde="";  /*险种*/
          c_undr_prsn="";   /*核保人*/
          c_undr_dpt="";    /*核保机构*/
          c_prn_typ="";     /*单证类型[0]='5'*/
          c_rinsrnc_mrk=""; /*分保类型*/
          c_coinsrnc_mrk="";/*共保标志*/
          c_ply_no="";      /*保单号*/
          c_edr_no="";      /*批单号*/
          c_read="";        /*读取标志  0-投保单未读入 1-投保单已读入 */
          c_flag="";        /*提交处理标志=7*/
          c_insrnt_nme="";  /*被保险人名称*/
          c_insrnc_cnm="";  /*险种名称*/
          c_dpt_cnm="";     /*所属机构名称*/
          c_send_dpt_cnm="";/*发送机构名称*/
          c_des_dpt_cnm=""; /*目标机构名称*/
          c_undr_dpt_cnm="";/*核保机构名称*/
          t_sign_tm="";     /*签单日期*/
          c_rpt_desc="";    /*上报说明*/
          c_edr_typ="";     /*批改类型*/
          c_risk_lvl_cde="";/*风险等级代码*/
          c_lcn_no="";      /*牌照号码*/
          c_eng_no="";      /*发动机号*/
          c_insrnc_cls="";  /*险种大类*/
          c_use_atr="";     /*使用性质*/
          c_app_typ="";     /*投保分类*/
          t_app_tm="";      /*投保日期*/
          t_input_tm="";    /*录入日期*/
          c_app_nme="";     /*投保人名称*/
          c_undr_opn="";    /*核保意见*/
          t_edr_app_tm="";  /*申请日期*/
          c_internet_typ="";/*网上业务标志[0]='2'*/


    }
}

...
<jsp:useBean id="prb"  scope="page" class="beans.PolicyPrintBean"/>
<jsp:setProperty name="prb" property="*"/>

<%
		 String condStr = prb.getPrintFormat("T_DOC_DATABLK");
...
%>
    <tr>
      <td width="13%" height="16"><a href="#" title="点击选择" onclick="selectNo(C_PLY_NO,<%=i-begini+1%>);return true"><%=Policy[i][c_ply_no]%></a></td>
...
%>
	     ...
	     <%@ include file="PageListNext.jsp" %>
          ...
          <tr>
            <td colspan="2" height="20"><%=PlyCnm%>号：
              <select name="C_PLY_NO" size="1">
                <option value="" selected>空<%=PlyCnm%>号</option>
                ...
                <%
                   if(PolicyLen > 1)
                   {
					  ...
					  String OpenCoverNoTemp ="";
     			      int c_open_cover_no = Objects.getColNum("c_open_cover_no",Policy);

					  int C_DPT_CDE =   Objects.getColNum("C_DPT_CDE",Policy);

			          int C_INSRNC_CDE = Objects.getColNum("C_INSRNC_CDE",Policy);
                      for(i = begini ;i< endi;i++ )
                      {
                          PolicyNoTemp = Policy[i][c_ply_no];
                          OpenCoverNoTemp = Policy[i][c_open_cover_no];
						  System.out.println("c_open_cover_no index :"+c_open_cover_no);

						  System.out.println("OpenCoverNoTemp:"+OpenCoverNoTemp);
                          if((!(OpenCoverNoTemp == null || OpenCoverNoTemp.equals(""))) & c_open_cover_no>0)
                               PolicyNoTemp = PolicyNoTemp+"&"+OpenCoverNoTemp;

                          if(AgencyNo.equals(Const.EDRSMTAPP)){

						     PolicyNoTemp = PolicyNoTemp + ","+ Policy[i][C_DPT_CDE] + ","

							   + Policy[i][C_INSRNC_CDE];

						  }
                %>
                          <option value= <%=PolicyNoTemp%>> <%=Policy[i][c_ply_no]%> </option>
                <%
                       }
                  }
                %>
  ...
  <%
  if(AgencyNo.equals(Const.FIND)  || AgencyNo.equals(Const.PRP))
  {//be to check if value is "" then not to modify
    if(AgencyNo.equals(Const.FIND))
    {
  %>
  <input type="submit" value="修改投保单" name="B12" onclick="IsNoticeModify(C_PLY_NO,New_baodan);setField(OPER,'PLYAPPMOD');return(checkFieldNull(C_PLY_NO,'0'));">
  <%}%>
  <input type="submit" value="查询建议书" name="B13" onclick="ChgAction(New_baodan,'ProposalFind.jsp');return(checkFieldNull(C_PLY_NO,'2'));">
  <%
  }
  else if(AgencyNo.equals(Const.CASE))
  {
     if(MenuRef.equals("MENU_REPORT"))
     {
  %>
  <input type="submit" value="报  案" name="B14" onclick="ChgAction(New_baodan,'Zinput_case.jsp');return true;">
     <%}else{%>
  <input type="submit" value="保单明细" name="B15" onclick="ChgAction(New_baodan,'Find_PlyView.jsp');return(checkFieldNull(C_PLY_NO,'4'));">
<select name="language" size="1">
        <option value="CHN" selected>中</option>
		<option value="ENG">英</option>
</select>
  <input type="submit" value="批改保单" name="B16" onclick="IsNoticeModify(C_PLY_NO,New_baodan);setField(OPER,'PLYAPPMOD');return(checkFieldNull(C_PLY_NO,'7'));">
  <input type="button" value="清单打印" name="B_PRINT" onclick="window.open('PolicyListPrint.jsp')">
  <%
             }
  }
  else if(AgencyNo.equals(Const.MODIFY))
  {%>
  <input type="submit" value="批改保单" name="B16" onclick="ChgAction(New_baodan,'ModifyPolicy.jsp');return true;">
  <%
  }else if(AgencyNo.equals(Const.EDRSMTAPP)){%>

    <input type="submit" value="修改申请单" name="B12" onclick="IsNoticeModify(C_PLY_NO,New_baodan);return(checkFieldNull(C_PLY_NO,'5'));">

	<!--<input type="submit" value="查询建议书" name="B13" onclick="ChgAction(New_baodan,'EdrsmtProsalFind.jsp');return(checkFieldNull(C_PLY_NO,'2'));">-->
  ...
  <%}else if(AgencyNo.equals(Const.EDRSMT)){%>
    ...
    <input type="submit" value="批单明细" name="B12" onclick="ChgAction(New_baodan,'EdrsmtView.jsp');return(checkFieldNull(C_PLY_NO,'6'));">
...
<input type="button" value="清单打印" name="B_PRINT" onclick="window.open('PolicyListPrint.jsp')">
  <%}else{
  %>
  <input type="submit" value="复制保单" name="B11" onclick="setField(OPER,'PLYAPPNEW');return(checkFieldNull(C_PLY_NO,'8'));;return true;">
  <%
  }
  %>
<!--

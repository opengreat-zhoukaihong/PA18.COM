...
/*
*****************************GENERAL INFORMATION***********************************************
*Program ID : Bus_Edr_List.jsp
*Directory : /data1/pa18/nul/mcip/public_html/operator_manager/
*URL : http://10.16.102.1:8800/operator_manager/Bus_Edr_List.jsp
...
/*E0002+*/
	...
	try{
		...
		bb.setFormTable(BConst.GET_EDR_PLCY_STATICTICS,formTable);
...
<div align="center"><br>
	<font color="#0066CC" class="unnamed3">批 单 清 单</font>
</div>
<table width="100%" border="1" cellspacing="0" cellpadding="0" class="td" bgcolor="#DFF9F3" align="center">
			<tr bgcolor="#9DEEDE">
				<td align="center" height="33" width="16%">机构</td>
				<td align="center" height="33" width="8%">险种</td>
				<td align="center" height="33" width="15%">批单号</td>
				<td align="center" height="33" width="13%">批改原因</td>
				<td align="center" height="33" width="9%">保费</td>
				<td align="center" height="33" width="10%">品名</td>
				<td align="center" height="33" width="8%">起始港 </td>
				<td align="center" height="33" width="5%">目的港</td>
				<td align="center" height="33" width="16%">出单日期</td>
			</tr>
	<% if (result != null && result.length>1){
			int indexEdrNo = Objects.getColNum("C_EDR_NO",result);
			int indexDptCde = Objects.getColNum("C_DPT_CDE",result);
			int indexInsrncCde = Objects.getColNum("C_INSRNC_CDE",result);
			int indexEdrRsn = Objects.getColNum("C_EDR_RSN",result);
			...
			int indexCrgoDesc = Objects.getColNum("C_CRGO_DESC",result);
			...
			int indexEdrRsnCnm = Objects.getColNum("C_EDR_RSN_CNM",result);
//			out.println("indexEdrRsn:"+indexEdrRsn+":indexEdrRsnCnm::"+indexEdrRsnCnm);
			for (int i=1;i<result.length;i++){%>
			<tr>
				<td align="center" width="16%"><%=result[i][indexDptCnm]%>&nbsp;</td>
				<td align="center" width="8%"><%=result[i][indexInsrncCnm]%>(<%=result[i][indexInsrncCde]%>)&nbsp;</td>
				<td align="center" width="15%"><%=result[i][indexEdrNo]%>&nbsp;</td>
				<td align="center" width="13%"><%=result[i][indexEdrRsnCnm]%>&nbsp;</td>
				<td align="center" width="9%"><%=util.fixedComma(result[i][indexNPrm])%>(<%=result[i][indexPrmCurCnm]%>)&nbsp;</td>
				<td align="center" width="10%"><%=result[i][indexCrgoDesc]%>&nbsp;</td>
				<td align="center" width="8%"><%=result[i][indexFromPrt]%>&nbsp;</td>
				<td align="center" width="5%"><%=result[i][indexToPrt]%>&nbsp;</td>
				<td align="center" width="16%"><%=result[i][indexUndrTm]%>&nbsp;</td>
			</tr>
	<%	}
	}%>
		</table>
	...
	<tr>
		...
		<%if (pNo.intValue()>0){%>
		<a href="Bus_Edr_List.jsp?numStr=<%=pNo.toString(0)%>">[首    页]</a>
		<a href="Bus_Edr_List.jsp?numStr=<%=pNo.toString(pNo.intValue()-1)%>">[上一页]</a>
		<%}
		else{
			out.println("[首    页]");
			out.println("[上一页]");
		...
		<a href="Bus_Edr_List.jsp?numStr=<%=pNo.toString(pNo.intValue()+1)%>">[下一页]</a>
		<a href="Bus_Edr_List.jsp?numStr=<%=pm.getTotalPage()-1%>">[末    页]</a>
		<%}
		else{
			out.println("[下一页]");
			out.println("[末    页]");
